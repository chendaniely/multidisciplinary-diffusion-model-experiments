...
create_plots <- function(dataframe, foldername){
    ...
    g.cos95 <- g + geom_line(aes(y = propCos95)) + ggtitle(foldername)
    g.cos95
    
    g.cos97 <- g + geom_line(aes(y = propCos97)) + ggtitle(foldername)
    g.cos97
    
    g.cos99 <- g + geom_line(aes(y = propCos99)) + ggtitle(foldername)
    g.cos99
    
    g.cos999 <- g + geom_line(aes(y = propCos999)) + ggtitle(foldername)
    g.cos999
...
for(pout_file in pout_files){
    ...
    plots <- create_plots(per_ever_updated_status[per_ever_updated_status$ifEverUpdated == 1, ], 
                          paste(foldername, 'updated_only', sep = '_'))
    plots_2 <- create_plots(per_time, paste(foldername, 'all', sep = '_'))

    # multiplot
    print(pout_file)
    multiplot(c(plots, plots_2), cols = 4) 
    # multiplot(plots_2, cols = 2)
}
```
