...
def main():
    ...
    print(clamp_strength)

    prototypes = config.get('LENSParameters', 'prototype')
    prototypes = mann.helper.convert_str_to_2d_int_array(prototypes)
    print(prototypes)

    prototype_mutation_p = config.getfloat('LENSParameters',
                                           'PrototypeMutationP')
    network_of_agents.\
        create_multidigraph_of_agents_from_edge_list(
            ...
            prototype_mutation_p=prototype_mutation_p
        )
    assert False, 'fail in main.py'
